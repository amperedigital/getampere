// v3.213: Console Log Pipe — sends browser console.log/warn/error to worker logs via /debug/console
// This lets the agent see both frontend and backend logs in a single observe.sh stream
(function () {
    const PIPE_URL = 'https://memory-api.tight-butterfly-7b71.workers.dev/debug/console';
    const FLUSH_MS = 2000;
    const MAX_BUFFER = 20;
    // Only capture logs with these prefixes (or all warnings/errors)
    const CAPTURE_PREFIXES = ['[AmpereChat]', '[AmpereVoice]', '[SystemLink]', '[Ampere Global]', '[AUTO-VOICEPRINT]', '[VoiceBuffer]'];
    let _buffer = [];
    let _flushing = false;
...
(function () {
    console.log('[Ampere Global] v3.212 Loaded');
...
function waitForUnicorn() {
    return new Promise((resolve) => {
        ...
        console.log("[Global] Waiting for Unicorn Studio...");
        let checks = 0;
...
async function initUnicorn() {
    // Robust Wait for Library (v3.149)
    if (!window.UnicornStudio) {
        console.log("[Unicorn] Library not found yet, waiting...");
        let libraryChecks = 0;
        await new Promise(resolve => {
            const libInterval = setInterval(() => {
                if (window.UnicornStudio || libraryChecks > 50) { // 50 checks * 100ms = 5s
    ...
    if (!window.UnicornStudio) {
        console.error("[Unicorn] Library not found on window after 5s.");
        return;
    }
    ...
    // --- Hero Background (Restored to First Position v3.157) ---
    ...
    if (heroTarget) {
        ...
        // Add artificial delay to prevent library race conditions (v3.157)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial check
    setTimeout(() => {
        console.log("[Global] Initializing v3.212...");
        checkNavTheme();
    }, 100);
});
    ...
    document.addEventListener('DOMContentLoaded', () => {
        waitForUnicorn().then(() => {
            ...
            if (keyContainers.length > 0) {
                ...
                // [Debug v3.100] Dynamic Absolute Import
                // Use the script's own URL to resolve the sibling, avoiding about:blank issues
                if (scriptUrl) {
                    componentUrl = scriptUrl.replace(/\/global.*?\.js$/, '/ampere-3d-key.js');
                } else {
                    // Fallback (unlikely to work if base is blank, but better than nothing)
