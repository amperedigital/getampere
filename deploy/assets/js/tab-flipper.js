...
});  });    }      }, 500);        isAnimating = false;      setTimeout(() => {      // Reset animation lock after transition (approx 500ms)      toggleAnimation(uc004Trigger, index === 3);      // Tab 3: ERP (UC_004)            toggleAnimation(uc003Trigger, index === 2);      // Tab 2: Scheduling (UC_003)            toggleAnimation(crmTrigger, index === 0);      // Tab 0: CRM (UC_001)      // Update SMIL Animations based on active tab      });        }          }            c.classList.add('inactive-next');          } else {            c.classList.add('inactive-prev');          if (i < index) {          // Determine direction for exit animation          }            video.currentTime = 0;            video.pause();          if (video) {          const video = c.querySelector('video');          // Pause video if present        } else {          if (video) video.play().catch(() => {});          const video = c.querySelector('video');          // Play video if present          c.classList.add('active');        if (i === index) {                c.classList.remove('active', 'inactive-prev', 'inactive-next');        // Reset classes      cards.forEach((c, i) => {      // Update Cards      });        }          t.classList.remove('active');          t.setAttribute('data-selected', 'false');          t.setAttribute('aria-selected', 'false');        } else {          t.classList.add('active');          t.setAttribute('data-selected', 'true');          t.setAttribute('aria-selected', 'true');        if (i === index) {      triggers.forEach((t, i) => {      // Update Triggers      activeIndex = index;      isAnimating = true;      console.log(`Setting active tab: ${index}`);    function setActive(index) {    });      });        setActive(index);        if (index === activeIndex || isAnimating) return;        e.preventDefault();      trigger.addEventListener('click', (e) => {    triggers.forEach((trigger, index) => {    // Event Listeners    setActive(0);    // Initialize    }        }            console.warn("SMIL animation control failed:", e);        } catch (e) {            }                triggerElement.endElement();            } else {                });                    triggerElement.beginElement();                requestAnimationFrame(() => {                // Small delay to ensure browser registers the stop                triggerElement.endElement();                 // Force restart            if (shouldRun) {        try {        if (!triggerElement) return;    function toggleAnimation(triggerElement, shouldRun) {    // Helper to toggle SMIL animations    const uc004Trigger = document.getElementById('uc004-anim-trigger');    const uc003Trigger = document.getElementById('uc003-anim-trigger');    const crmTrigger = document.getElementById('crm-anim-trigger');    // Animation Triggers        let isAnimating = false;    let activeIndex = 0;    // State    if (!triggers.length || !cards.length) return;        const cards = flipper.querySelectorAll('[data-tab-card]');    const triggers = flipper.querySelectorAll('[data-tab-trigger]');  flippers.forEach(flipper => {  const flippers = document.querySelectorAll('[data-tab-flipper]');  });    });      el.appendChild(span);      span.style.transitionDelay = `${i * 30}ms`;      span.classList.add('char');      span.textContent = char;      const span = document.createElement('span');    [...text].forEach((char, i) => {    el.innerHTML = '';    const text = el.textContent;  flipTexts.forEach(el => {  const flipTexts = document.querySelectorAll('.hover-flip-text');  // Initialize Text Flip Effect  console.log('Tab Flipper v2.1 Loaded');document.addEventListener('DOMContentLoaded', () => { */ * Manages SMIL animations for 3D cards based on active tab state. * Supports 3D transitions via CSS classes managed by this script. * Handles the switching of active states between navigation tabs and corresponding content cards. * Tab Controlled Card Flipper v2.1
...
});  });    }      }, 500);        isAnimating = false;      setTimeout(() => {      // Reset animation lock after transition (approx 500ms)      toggleAnimation(uc004Trigger, index === 3);      // Tab 3: ERP (UC_004)            toggleAnimation(uc003Trigger, index === 2);      // Tab 2: Scheduling (UC_003)            toggleAnimation(crmTrigger, index === 0);      // Tab 0: CRM (UC_001)      // Update SMIL Animations based on active tab      });        }          }            c.classList.add('inactive-next');          } else {            c.classList.add('inactive-prev');          if (i < index) {          // Determine direction for exit animation          }            video.currentTime = 0;            video.pause();          if (video) {          const video = c.querySelector('video');          // Pause video if present        } else {          if (video) video.play().catch(() => {});          const video = c.querySelector('video');          // Play video if present          c.classList.add('active');        if (i === index) {                c.classList.remove('active', 'inactive-prev', 'inactive-next');        // Reset classes      cards.forEach((c, i) => {      // Update Cards      });        }          t.classList.remove('active');          t.setAttribute('data-selected', 'false');          t.setAttribute('aria-selected', 'false');        } else {          t.classList.add('active');          t.setAttribute('data-selected', 'true');          t.setAttribute('aria-selected', 'true');        if (i === index) {      triggers.forEach((t, i) => {      // Update Triggers      activeIndex = index;      isAnimating = true;    function setActive(index) {    });      });        setActive(index);        if (index === activeIndex || isAnimating) return;        e.preventDefault();      trigger.addEventListener('click', (e) => {    triggers.forEach((trigger, index) => {    // Event Listeners    setActive(0);    // Initialize    }        }            console.warn("SMIL animation control failed:", e);        } catch (e) {            }                triggerElement.endElement();            } else {                triggerElement.beginElement();            if (shouldRun) {        try {        if (!triggerElement) return;    function toggleAnimation(triggerElement, shouldRun) {    // Helper to toggle SMIL animations    const uc004Trigger = document.getElementById('uc004-anim-trigger');    const uc003Trigger = document.getElementById('uc003-anim-trigger');    const crmTrigger = document.getElementById('crm-anim-trigger');    // Animation Triggers        let isAnimating = false;    let activeIndex = 0;    // State    if (!triggers.length || !cards.length) return;        const cards = flipper.querySelectorAll('[data-tab-card]');    const triggers = flipper.querySelectorAll('[data-tab-trigger]');  flippers.forEach(flipper => {  const flippers = document.querySelectorAll('[data-tab-flipper]');  });    });      el.appendChild(span);      span.style.transitionDelay = `${i * 30}ms`;      span.classList.add('char');      span.textContent = char;      const span = document.createElement('span');    [...text].forEach((char, i) => {    el.innerHTML = '';    const text = el.textContent;  flipTexts.forEach(el => {  const flipTexts = document.querySelectorAll('.hover-flip-text');  // Initialize Text Flip Effectdocument.addEventListener('DOMContentLoaded', () => { */ * Manages SMIL animations for 3D cards based on active tab state. * Supports 3D transitions via CSS classes managed by this script. * Handles the switching of active states between navigation tabs and corresponding content cards. * Tab Controlled Card Flipper
